...
func CheckUserName(name string) int  {
	...
	if user.ID > 0{
		return status_msg.ERROR_USERNAME_USED // 1001
	}
	return status_msg.SUCCSE // 200
}
...
func GetUser(id int) (User, int)  {
	...
	if err != nil {
		return user, status_msg.ERROR
	}
	return user, status_msg.SUCCSE
}
...
func CheckLogin(username string, password string) (User, int) {
	...
	if user.ID == 0 { // 用户不存在
		return user, status_msg.ERROR_USER_NOT_EXIST
	}
	if user.Role != 1 { // 非管理员
		return user, status_msg.ERROR_USER_NO_RIGHT
	}

	PasswordErr = bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(password))

	if PasswordErr != nil {
		return user, status_msg.ERROR_PASSWORD_WRONG
	}
	return user, status_msg.SUCCSE
}
