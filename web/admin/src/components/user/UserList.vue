<template>
  <div>
    ...
    <a-modal
      ...
      destroyOnClose
    >
      <a-form-model :model="checkPwd" :rules="checkPwdRules" ref="changePasswordRef">
        <a-form-model-item has-feedback label="原密码" prop="password">
          <a-input-password v-model="checkPwd.password"></a-input-password>
        </a-form-model-item>
        <a-button @click="checkPassword">确认</a-button>
      </a-form-model>
...
<script>
  ...
  export default {
          ...
          password: [
            {
              validator: (rule, value, callback) => {
                ...
                } else if ([...this.checkPwd.password].length < 6 || [...this.checkPwd.password].length > 20) {
          ...
          password: [
            {
              validator: (rule, value, callback) => {
                ...
                } else if (this.changePassword.password == '') {
                  callback(new Error('请输入密码'))
                } else if ([...this.changePassword.password].length < 6 || [...this.changePassword.password].length > 20) {
                  callback(new Error('密码应当在6到20位之间'))
                } else {
                  callback()
                }
          ...
          checkpass: [
            {
              validator: (rule, value, callback) => {
                if (!this.isChecked) {
                  callback(new Error('请输入正确的原密码'))
                } else if (this.changePassword.checkpass == '') {
                  callback(new Error('请输入密码'))
                }else if (this.changePassword.password !== this.changePassword.checkpass) {
                  callback(new Error('密码不一致，请重新输入'))
                } else {
                  callback()
                }
    ...
    methods: {
      ...
      async checkPassword(){
        const { data: res } = await this.$http.post('login', {username: this.checkPwd.username, password:this.checkPwd.password})
        if (res.status != 200) return this.$message.error(res.message)
        this.isChecked = true
      ...
      async ChangePassword(id) {
        ...
        this.newPwdVisible = false // 先隐藏新密码输入
        this.isChecked = false
        const { data: res } = await this.$http.get(`user/${id}`) // 获取用户信息用于校验原密码
        this.checkPwd.username = res.data.username
